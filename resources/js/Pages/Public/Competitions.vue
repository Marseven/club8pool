...
const props = defineProps({
  grouped: Object,   // { in_progress: [], registration: [], draft: [], finished: [] }
  total: Number,
});
...
const STATUS_META = {
  in_progress:  { label: 'EN COURS',      chip: 'live',    chipText: 'EN COURS' },
  registration: { label: 'INSCRIPTIONS',  chip: '',        chipText: 'INSCRIPTIONS OUVERTES' },
  draft:        { label: 'À VENIR',       chip: '',        chipText: 'À VENIR' },
  finished:     { label: 'TERMINÉES',     chip: 'muted',   chipText: 'TERMINÉE' },
};

const STATUS_SECTIONS = [
  { key: 'in_progress',  title: 'En cours' },
  { key: 'registration', title: 'Inscriptions ouvertes' },
  { key: 'draft',        title: 'À venir' },
  { key: 'finished',     title: 'Archives' },
];
...
const hasAny = (key) => props.grouped?.[key]?.length > 0;
const hasCompetitions = STATUS_SECTIONS.some(s => hasAny(s.key));
</script>

<template>
  <Head title="Compétitions">
    <meta name="description" content="Toutes les compétitions Club 8 Pool — en cours, inscriptions ouvertes et archives." head-key="description" />
  </Head>
  ...
  <div style="background: var(--ink); min-height: 100vh;">
    ...
    <section style="padding: 32px 0; border-bottom: 1px solid var(--line);">
      <div class="container">
        <div class="mono" style="font-size: 11px; letter-spacing: 0.22em; color: var(--mute);">COMPÉTITIONS</div>
        <h1 class="disp-a" style="font-size: clamp(40px, 9vw, 80px); margin-top: 14px; line-height: 0.92;">
          Toutes<br /><span style="color: var(--mute);">les compétitions</span>
        </h1>
        <p v-if="total > 0" class="mono" style="font-size: 12px; color: var(--mute); margin-top: 14px; letter-spacing: 0.16em;">
          {{ total }} COMPÉTITION{{ total > 1 ? 'S' : '' }} AU TOTAL
        </p>
      </div>
    </section>

    <!-- Empty state -->
    <section v-if="!hasCompetitions" style="padding: 48px 0;">
      <div class="container">
        <div style="padding: 40px; border: 1px dashed var(--line-strong); text-align: center; max-width: 560px; margin: 0 auto;">
          <div class="disp-a" style="font-size: clamp(20px, 5vw, 28px); color: var(--mute);">Aucune compétition</div>
          <p style="font-size: 13px; color: var(--mute); margin-top: 14px; line-height: 1.6;">
            Les compétitions apparaîtront ici dès leur création.
          </p>
        </div>
      </div>
    </section>
    ...
    <!-- Sections par statut -->
    <template v-for="section in STATUS_SECTIONS" :key="section.key">
      <section v-if="hasAny(section.key)" style="padding: 32px 0; border-bottom: 1px solid var(--line);">
        <div class="container">
          <div class="mono" style="font-size: 10px; letter-spacing: 0.22em; color: var(--mute); margin-bottom: 20px;">
            {{ section.title.toUpperCase() }}
          </div>

          <div class="c-grid" style="display: grid; grid-template-columns: repeat(2, 1fr); gap: 18px;">
            <Link
              v-for="c in grouped[section.key]"
              :key="c.id"
            ...
            >
              ...
              <div style="display: flex; align-items: center; gap: 8px; flex-wrap: wrap;">
                <Chip :variant="STATUS_META[section.key]?.chip ?? ''">
                  {{ STATUS_META[section.key]?.chipText ?? section.key.toUpperCase() }}
                </Chip>
              ...
              <div style="display: flex; gap: 20px; margin-top: 16px; border-top: 1px solid var(--line); padding-top: 14px;">
                <div>
                  <span class="disp-a tnum" style="font-size: 24px; color: var(--chalk);">{{ c.registrations_count ?? 0 }}</span>
                  <span style="font-size: 11px; color: var(--mute); margin-left: 4px;">joueurs</span>
                </div>
                <div>
                  <span class="disp-a tnum" style="font-size: 24px; color: var(--chalk);">{{ c.matches_count ?? 0 }}</span>
